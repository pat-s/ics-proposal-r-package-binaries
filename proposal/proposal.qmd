...
Establish an **open-source** build system for R package binaries.
Not only the source code should be public, but also the actual build processes (running in CI) should be publicly accesible.
...
The engine should be written in a generic way, so that it can be used to build binaries from any R package source, being it a local or remote one.
